...
export function SecureVault({ clientId }: { clientId?: string }) {
  ...
  const filtered = items.filter(v => `${v.serviceName} ${v.username} ${v.notes || ""}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase()));

  useEffect(() => () => timers.current.forEach(clearTimeout), []);

  async function show(id: string, copy = false) {
    if (!copy && visible[id] !== undefined) {
      setVisible(v => { const next = { ...v }; delete next[id]; return next; });
      return;
    }
    try {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingId) {
        ...
        setVisible(v => { const next = { ...v }; delete next[editingId]; return next; });
      }
      else if (clientId) await app.addClientVaultItem({ ...data, clientId, category: "otro" });
      else await app.addVaultCredential({ ...data, serviceType: "otro", allowedRoles: [app.currentUser.role] });
      form.reset();
      setEditing(false);
      setEditingId(null); setError(""); showSuccess("Guardado", "Credencial guardada correctamente.");
    } catch (e) {
  ...
  return (
    <section className="space-y-4 select-none">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          disabled={busy} onClick={() => { setEditing(!editing); setEditingId(null); setError(""); }}
      ...
      {editing && (
        <form key={editingId || "new"} onSubmit={submit} className="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-lumina-card grid gap-4 w-full max-w-4xl">
          <h3 className="font-bold text-xs text-[#0b1c30] flex items-center gap-2 border-b pb-2">
            <Lock className="w-4 h-4 text-[#004ac6]" />
            {editingId ? "Editar credencial" : "Registrar Nuevo Acceso Seguro"}
          </h3>
          <div className="grid sm:grid-cols-2 gap-4">
            <label className="text-xs font-bold text-[#434655]">
              Servicio / Destino
              <input name="service" defaultValue={edited?.serviceName || ""} required maxLength={200} placeholder="Ej: Router Core CCR, OLT Huawei..." className="mt-1 block border border-[#cbd5e1] rounded-lg p-2.5 w-full text-xs font-medium text-[#0b1c30]" />
            </label>
            <label className="text-xs font-bold text-[#434655]">
              Usuario / Identificador
              <input name="username" defaultValue={edited?.username || ""} required maxLength={200} autoComplete="off" placeholder="admin_noc" className="mt-1 block border border-[#cbd5e1] rounded-lg p-2.5 w-full text-xs font-medium text-[#0b1c30]" />
            </label>
          </div>
          <label className="text-xs font-bold text-[#434655]">
            Contraseña {editingId ? "(dejar vacía para conservar la actual)" : ""}
            <input name="password" required={!editingId} type="password" maxLength={4096} autoComplete="new-password" placeholder="••••••••••••" className="mt-1 block border border-[#cbd5e1] rounded-lg p-2.5 w-full text-xs font-medium text-[#0b1c30]" />
          </label>
          <label className="text-xs font-bold text-[#434655]">
            Notas / Referencia técnica
            <textarea name="notes" defaultValue={edited?.notes || ""} maxLength={2000} placeholder="Detalles de puerto, IP privada, etc." className="mt-1 block border border-[#cbd5e1] rounded-lg p-2.5 w-full text-xs font-medium text-[#0b1c30]" rows={2} />
          </label>
          ...
          <div className="flex gap-3">
            ...
            <button disabled={busy} type="button" onClick={() => setEditing(false)} className="px-4 py-2.5 rounded-lg text-xs font-bold text-[#737686] hover:bg-slate-100">
              Cancelar
            </button>
          </div>
        </form>
      ...
      <label className="block text-sm">Filtrar credenciales<input value={query} onChange={e => setQuery(e.target.value)} placeholder="Servicio, usuario o notas" className="mt-1 block w-full rounded-lg border p-3" /></label>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-5 py-10 text-center text-xs italic text-[#737686]">
                  No hay credenciales que coincidan. Limpia el filtro o registra una nueva credencial.
                </td>
              </tr>
            ) : filtered.map((item) => (
              <tr key={item.id} className="hover:bg-[#f8f9ff]">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center justify-end gap-3 whitespace-nowrap">
                    <button disabled={busy} onClick={() => { setEditingId(item.id); setEditing(true); setError(""); }} className="text-blue-700 font-bold">Editar</button>
                    <button
